...
function showFeedBack(el, str, error=true, url=null){
    ...
    if(url != null){
        if(!error){
            var newDiv = $('<div/>').addClass('alert alert-success flush').html('<h5><i class="fa fa-check-circle"></i> ' + str + '</h5>').delay(2000).fadeOut('normal', function(){
                window.location = url;
            });
        }else{
            var newDiv = $('<div/>').addClass('alert alert-danger flush').html('<h5><i class="fa fa-close"></i> ' + str + '</h5>').delay(2000).fadeOut('normal', function(){
                window.location = url;
            });
        }
        $(el).before(newDiv);
